...
from ..schemas.user_schemas import CreateUser, UserLogin, UpdateUser, ChangePassword
...
async def signup(payload:CreateUser):
    new_user = users.create_user(payload)
    return {"data":new_user, "message":"user created successfully"}

@router.post("/login", status_code=status.HTTP_200_OK)
async def login(payload:UserLogin):
    user_login = users.user_login(payload)
...
async def update_profile(user_id:int, payload:UpdateUser):
    updated_user = users.update_user(user_id, payload)
